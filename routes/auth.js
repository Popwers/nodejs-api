...
module.exports = db => {
    ...
    router
        .post('/login', fieldsMiddleware, async (req, res) => {
            ...
            else if (!user.results.length > 0) res.status(409).send("Votre identifiant ou votre mot de passe est incorrect.")
            else user = user.results[0]

            // Check if password is correct
            if (await bcrypt.compare(password, user.password)) {
                const token = jwt.sign({ user_id: user.id, mail: user.mail }, process.env.TOKEN_KEY, { expiresIn: "30 days", })
        ...
        .post('/signup', [authMiddleware, fieldsMiddleware], async (req, res) => {
            ...
            else if (newUser.results?.affectedRows && newUser.results?.affectedRows > 0) res.status(200).send("Compte utilisateur enregistré.")
